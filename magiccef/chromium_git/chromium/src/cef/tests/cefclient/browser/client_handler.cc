...
bool ClientHandler::OnProcessMessageReceived(
  ...
  if (message_name == kFocusedNodeChangedMessage) {
    ...
    return true;
  ...
  else if (message_name == "pca_mouseevent") {
      auto args = message->GetArgumentList();
      CefString para = args->GetString(0);
      std::wstring s = para.c_str();
      if (para == "leftclick") {
      ...
      if(p1 == std::string::npos)
         return true;
      auto s_x = s.substr(0, p1);
      auto s_y = s.substr(p1 + 1, s.length() - p1 - 1);
      CefMouseEvent mouse_event;
      std::wistringstream ss1(s_x);
      int x;
      ss1 >> x;

      std::wistringstream ss2(s_y);
      int y;
      ss2 >> y;
      //ShowCursor(FALSE);
      SetCursorPos(x, y);
